            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2.5 rounded-xl text-left text-xs font-semibold transition-all duration-200 flex items-center justify-between gap-2 ${isActive
                ...
                >
                  ...
                  <span className={`text-[10px] px-2 py-0.5 rounded-full font-mono font-bold ${isActive ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'
                    }`}>
                    {count}
                  </span>
                </button>
          ...
          <div className="md:col-span-9 relative w-full aspect-[1200/600] rounded-3xl bg-white/30 border border-white/60 backdrop-blur-sm shadow-[inset_0_2px_10px_rgba(255,255,255,0.6)] overflow-visible">
            ...
            {technologiesList.map((tech) => {
              ...
              return (
                <div
                  key={tech.id}
                  className={`absolute z-20 transition-all duration-500 ${isMatch ? 'opacity-100 scale-100' : 'opacity-20 scale-90 grayscale pointer-events-none'
                    }`}
                  style={{
                ...
                >
                  <div className={`flex flex-col gap-0.5 px-3 py-1.5 lg:px-3.5 lg:py-2 bg-white/95 backdrop-blur-md rounded-[16px] shadow-[0_6px_18px_rgba(0,0,0,0.06)] border ${isMatch ? 'border-white hover:border-slate-300' : 'border-slate-200'
          ...
          <div className="flex flex-wrap justify-center gap-1.5 p-1.5 bg-white/80 rounded-2xl border border-white max-w-[360px] shadow-sm">
            {categories.map((cat) => (
              <button
                key={cat.id}
                onClick={() => setActiveCategory(cat.id)}
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-colors ${activeCategory === cat.id ? 'bg-[#0f172a] text-white' : 'text-slate-600'
                  }`}
              >
                {cat.label}
              </button>
          ...
          <div className="flex flex-wrap justify-center gap-2 max-w-[380px] px-2">
            {technologiesList.map((item, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className={`bg-white/95 backdrop-blur-md px-3.5 py-1.5 rounded-full shadow-[0_4px_12px_rgba(0,0,0,0.05)] border border-white flex items-center gap-2 text-[12px] font-semibold text-slate-800 transition-all duration-300 ${isMatch ? 'opacity-100 scale-100' : 'opacity-30 scale-95 grayscale'
                    }`}
                >
